...
public class ParallelTest {
    ...
    // 디버깅 어려움 : 병렬 스트림은 여러 스레드가 동시에 처리하기 힘들기 때문에 디버깅이 어려울 수 있다.

    @Test
    void sequentialVsParallel() {
        List<Integer> numbers = IntStream.range(0, 10).boxed().toList();

        long sequrntialStartAt = System.currentTimeMillis();
        numbers.forEach(
                i -> {
                    try {
                        Thread.sleep(100);
        ...
        System.out.println("Sequential : " +  (System.currentTimeMillis() - sequrntialStartAt));
